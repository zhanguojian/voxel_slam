...
class PangolinWindowImpl {
 public:
  ...
  /// 初始化，创建各种点云、小车实体
  bool init(const std::string &win_name);
 ...
 public:
  ...
  std::atomic<bool> cloud_static_global_need_update_;
  ...
  /// @brief 静态全局点云
  CloudPtr cloud_static_global_map_;
  ...
  SE3 T_imu_lidar_;
  uint32_t max_size_of_current_scan_ = 2000;  // 当前扫描数据保留多少个
 ...
 private:
  ...
  std::string win_name_ = "slam_tools";
