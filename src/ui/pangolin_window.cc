...
bool PangolinWindow::init(const std::string& name) {
...
void PangolinWindow::updateStaticPointCloudGlobal(const CloudPtr cloud) {
  std::lock_guard<std::mutex> lock(impl_->mtx_map_cloud_);
  impl_->cloud_static_global_map_ = cloud;
  impl_->cloud_static_global_need_update_.store(true);
}
